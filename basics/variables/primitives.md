...
A partir de celles-ci, on crée d'autre variables : [**les Objets**](objects.md).
...
{% method -%}
On voit que pour **certains type** de variables, on a besoin de spécifier le type avec une lettre derrière la valeur.  
Par défaut, chaque nombre est de type int. Pour cela, on va [convertir](cast.md) les objets.
{% sample lang="java" -%}
```java
byte b = (byte) 12; // On convertit le nombre en byte
short s = (short) 32_000; // De même en shot
int i = 235_000; // On définit un entier normal
long l = 372_036_854_775_800L;// On spécifie le type long (grand nombre). Sans L, le code ne fonctionnera pas.
